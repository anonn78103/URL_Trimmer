...
app.use(helmet());
app.use(cors({
  origin: ['https://url-trimmer.01k.in', 'https://localhost:80'],
  credentials: true
}));
...
res.redirect(`https://url-trimmer.01k.in/auth-callback?token=${token}&userId=${user._id}&name=${encodeURIComponent(user.name)}&email=${encodeURIComponent(user.email)}`);
});
